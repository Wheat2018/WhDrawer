...
#define Operator3D(p1, p2, opt) {p1.x opt p2.x, p1.y opt p2.y, p1.z opt p2.z};
	...
	static _Rel Relation(_P2<T1> point1, _P2<T2> point2)
	{
	...
	/// <returns>几何关系枚举。None或Coincident。</returns>
	template<typename T1, typename T2>
	static _Rel Relation(_P3<T1> point1, _P3<T2> point2)
	{
	...
	static _Rel Relation(_P2<PT> point, _L2<LT> line)
	{
		/*
			实现逻辑：代入一般式。
		*/
	...
	/// <returns>几何关系枚举。None或Contained。</returns>
	template<typename PT, typename LT>
	static _Rel Relation(_P3<PT> point, _L3<LT> line)
	{
		/*
	...
	template<typename T1, typename T2>
	static _Rel Relation(_L2<T1> line1, _L2<T2> line2)
	{
		/*
	...
	template<typename T1, typename T2>
	static _Rel Relation(_L3<T1> line1, _L3<T2> line2)
	{
		/*
	...
	static _Rel Relation(_P3<PT> point, _E<ET> plane)
	{
		/*
			实现逻辑：代入一般式。
		*/
	...
	static _Rel Relation(_L3<LT> line, _E<ET> plane)
	{
		/*
	...
	/// 二维点欧氏距离。
	/// </summary>
	/// <typeparam name="Ty">点数据类型。</typeparam>
	/// <param name="point1">点1。</param>
	/// <param name="point2">点2。</param>
	/// <returns></returns>
	template<typename T>
	static float Distance(_P2<T> point1, _P2<T> point2)
	{
	...
	/// <returns></returns>
	template<typename T>
	static float Distance(_P3<T> point1, _P3<T> point2)
	{
		return sqrt(Pow2(point1.x - point2.x) + Pow2(point1.y - point2.y) + Pow2(point1.z - point2.z));
	}
